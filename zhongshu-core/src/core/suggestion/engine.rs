...
impl SuggestionEngine {
    pub fn new(db: Database) -> Self {
        SuggestionEngine { db, min_confidence: 0.5 }
    }

    pub fn insert(&self, content: &str, type_: Option<&str>, confidence: f64, source_obs: Option<&str>) -> rusqlite::Result<Suggestion> {
    ...
    pub fn analyze(&self) -> rusqlite::Result<Vec<Suggestion>> {
        ...
        let recent: Vec<Observation> = {
            ...
            let rows = stmt.query_map(params![now() - 3600 * 24], |row| {
                Ok(Observation {
                    id: row.get(0)?,
        ...
        let user_msgs: Vec<&Observation> = recent.iter().filter(|o| matches!(o.type_, ObservationType::UserMessage)).collect();
        if user_msgs.len() >= 3 {
        ...
        let failures: Vec<&Observation> = recent.iter().filter(|o| matches!(o.type_, ObservationType::ToolResult)).collect();
        if !failures.is_empty() {
            created.push(self.insert("检查近期工具执行失败，确认是否需要人工介入", Some("investigate"), 0.3, None)?);
        }
    ...
    pub fn list_pending(&self) -> rusqlite::Result<Vec<Suggestion>> {
        let conn = self.db.conn()?;
        let mut stmt = conn.prepare(
            "SELECT id, type, content, confidence, status, source_observation, created_at FROM suggestions WHERE status = 'pending' ORDER BY confidence DESC",
        )?;
    ...
    pub fn get_content(&self, id: &str) -> rusqlite::Result<String> {
        let conn = self.db.conn()?;
        conn.query_row("SELECT content FROM suggestions WHERE id = ?1", params![id], |row| row.get(0))
    }

    pub fn update_status(&self, id: &str, status: &SuggestionStatus) -> rusqlite::Result<bool> {
        let conn = self.db.conn()?;
        let n = conn.execute("UPDATE suggestions SET status = ?1 WHERE id = ?2", params![status.as_str(), id])?;
        Ok(n > 0)
    }
    ...
    fn row(row: &rusqlite::Row) -> rusqlite::Result<Suggestion> {
        Ok(Suggestion {
            ...
            status: SuggestionStatus::from_str(&row.get::<_, String>(4)?).unwrap_or(SuggestionStatus::Pending),
