...
pub fn requires_approval(risk: EffectRisk) -> bool {
    matches!(
        risk,
        EffectRisk::DestructiveMutation
            | EffectRisk::ExternalSideEffect
            | EffectRisk::SensitiveDataAccess
    )
}

pub fn check_and_set_pending(tool_name: &str, command: &str, risk: EffectRisk) -> bool {
