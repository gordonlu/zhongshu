...
pub fn build_report(events: &[HarnessEvent]) -> String {
    ...
    let tool_calls = events.iter().filter(|e| matches!(e, HarnessEvent::ToolCall { .. })).count();
    let verifications = events.iter().filter(|e| matches!(e, HarnessEvent::Verification { .. })).count();
    format!(
